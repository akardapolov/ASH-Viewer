...
import org.ash.detail.GanttDetails;
import org.ash.gui.Gantt;
import org.ash.gui.StackedChart;
import org.ash.gui.StatusBar;
import org.ash.history.GanttH;
import org.ash.history.TopActivityDetail;
...
public class ChartPanel extends JPanel implements ChartChangeListener,
    ...
    public static final int DEFAULT_ZOOM_TRIGGER_DISTANCE = 8;
    ...
    /** The timestamp of domain, left side of the chart. */
    private double dLeft;

    /** The timestamp of domain, right side of the chart. */
    ...
    /** A flag that controls whether or not selection done. */
    private boolean clearingLastRestangle2d = false;
     ...
     * Add by akarapolov
     */
	private Rectangle2D zoomArea;
	private boolean bMouseClicked = false;
	private boolean bisMouseDragged = false;
    ...
    /**
     * Set the flag that determines whether or not multiple selection is enabled for
     * the domain axis.
     *
     */
    ...
    /**
     * Returns the flag that determines whether or not multiple selection is enabled for
     * the domain axis.
     *
     * @return A boolean.
     */
    ...
    public void mouseDragged(MouseEvent e) {
    	...
    	// Set mouse dragged
    	this.bisMouseDragged = true;
    	
    	// Exit when zoomable disabled
    	if (!this.domainZoomable){
    		return;
    	}
    	
    	// Redraw chart (exclude multiple rect)
    	if (clearingLastRestangle2d){
    		clearingLastRestangle2d = false;
    		this.zoomArea = null;
    	}
        ...
        if (this.zoomRectangle != null) {
            ...
            if (zoomTrigger1 || zoomTrigger2) {
                    ...
                    double maxY = screenDataArea.getMaxY();
                      ...
                      zoomArea = new Rectangle2D.Double(x, y, w, h);
            	
                    // Get dLeft and dRiht of selection rectangle
                    zoom(zoomArea);
                    
                    //send event for sql & session lists
                    fireReleaseMouse();
                ...
                this.zoomPoint = null;
                this.zoomRectangle = null;
                this.clearingLastRestangle2d = true;
            }
            else {
    ...
    /**
	 * Draws rectangle when redraw.
	 * 
	 * @param g2
	 *            the graphics device.
	 */
	private void drawScaledRectangle(Graphics2D g2) {
		// Set XOR mode to draw the zoom rectangle
		g2.setXORMode(Color.gray);
		if (this.zoomArea != null ) {
			...
			if (p instanceof XYPlot) {
				...
				if (!this.bMouseClicked) {
					...
					x = z.getValueToJava2d(this.dLeft, plotInfo);
					y = screenDataArea.getMinY();
					w = z.getValueToJava2d(this.dRight, plotInfo)
						- z.getValueToJava2d(this.dLeft, plotInfo);
					h = screenDataArea.getHeight();
					if (this.getScaleX() == 1.0){
					   this.zoomArea = new Rectangle2D.Double(x, y, w, h);
					} else {
						this.zoomArea = 
							new Rectangle2D.Double(x*this.getScaleX(), y, 
												   w*this.getScaleX(), h);
					}
				}
			...
			if (this.fillZoomRectangle) {
				g2.fill(this.zoomArea);
			} else {
				g2.draw(this.zoomArea);
			}
		}
		// Reset to the default 'overwrite' mode
		g2.setPaintMode();
     ...
     * @return boolean
     */
    public boolean removeListenerReleaseMouse(Object l) {
     ...
     * @param l
     */
    public void addListenerReleaseMouse(Object l) {
    ...
    protected void fireReleaseMouse() {
        ...
        while (iStart.hasNext()) {
      	  Object 
      	  	currListeners = (Object) iStart.next();
      	  // For StackedXYAreaChart update data label (dd.MM.yyyy format)
      	  if (currListeners instanceof Gantt){
      		Gantt 
      		  	tempObj = (Gantt) currListeners;
      		  	tempObj.loadDataToJPanels(this.dLeft, this.dRight);
      	  }
      	 
      	if (currListeners instanceof GanttDetails){
      		GanttDetails 
      		  	tempObj = (GanttDetails) currListeners;
      		  	tempObj.loadDataToJPanels(this.dLeft, this.dRight);
      	  }
      	...
      	if (currListeners instanceof StatusBar){
      		...
      		if (this.getChart().getTitle().getText().equalsIgnoreCase("Top activity")){
      		  	tempObj.setRange(this.dLeft, this.dRight);
      		  	tempObj.setSelection("Manual");
      		} else {
      			tempObj.setLabelForDetailChart(this.getChart().getTitle().getText(), 
      					this.dLeft, this.dRight);
      		}
      	  }
      	
      	if (currListeners instanceof TopActivityDetail){
      		TopActivityDetail 
  		  		tempObj = (TopActivityDetail) currListeners;
      		tempObj.loadPreviewStackedChart(this.dLeft, this.dRight);
      	}
      	
      	 if (currListeners instanceof GanttH){
      		GanttH 
       		  	tempObj = (GanttH) currListeners;
       		  	tempObj.loadDataToJPanels(this.dLeft, this.dRight);
       	  }
      	 
      	if (currListeners instanceof GanttDetailsH){
      		GanttDetailsH 
      		  	tempObj = (GanttDetailsH) currListeners;
      		  	tempObj.loadDataToJPanels(this.dLeft, this.dRight);
      	  }
    ...
    /** listener */
    private List listenersStart = Collections.synchronizedList(new ArrayList());

	/**
	 * @return the isMouseDragged
	 */
	public boolean isMouseDragged() {
		return bisMouseDragged;
	}
