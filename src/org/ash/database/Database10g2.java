/*
 *-------------------
 * The Database10g2.java is part of ASH Viewer
 *-------------------
 * 
 ...
 */
public class Database10g2 extends ASHDatabase {
	...
	/** The query sql. */
	private String querySQL = "SELECT sql_id, command_type, sql_text "
	 ...
	 *            the BDB dao
	 */
	public Database10g2(Model model0) {
		super(model0);
		this.model = model0;
	 ...
	 */
	public void loadToLocalBDBCollector() {
		// Get max value of ash
		super.initializeVarsOnLoad();
		...
		try {
			...
			if (model.getConnectionPool() != null) {
				...
				while (resultSetAsh.next()) {
					...
					if (isReconnect()) {
						setKReconnect(sampleIdTmp - super.getSampleId());
						setReconnect(false);
					}
					...
					try {
						...
						dao.activeSessionHistoryById
								.putNoReturn(new ActiveSessionHistory(
										...
										0.0, 0.0,
										resultSetAsh.getString("XID"), 0.0, "",
										"", "", "", "", "", "", "", "", "",
