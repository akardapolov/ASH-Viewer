...
public class Collector9iAndSEDB implements Runnable, Collector {
  ...
  /** The range window. */
  private int rangeWindow = 900000; // 15 minutes default
  
  /** The k. 1 minute */
  private int k = 60000;
  
  /**
   * Instantiates a new Collector9iLowerDB.
   * 
 ...
 */
  ...
  public void run() {
	    ...
	    database.loadToLocalBDBCollector();
	      ...
	      try {
	        Thread.sleep(Math.max(m_latencyBDBCollector - System.currentTimeMillis() + lasttime, 0));
